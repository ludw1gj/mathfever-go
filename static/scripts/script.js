...
function validateForm(formData) {
    ...
    for (var key in formData) {
        // removes all whitespace from the form model
        ...
        if (formData[key] === '') {
            ...
            if (!decimalValidation(formData[key])) {
                ...
                // model must be an integer string not a float string
    ...
    // if the model is valid, send the Ajax request
    if (isValidInput) {
...
/**
 * This function submits an ajax request of content-type json to the 'api' router
 * @param {Object} data
